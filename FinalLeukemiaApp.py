...
def save_confidence_plot(prediction):
    fig, ax = plt.subplots(figsize=(8, 4))
    indices = np.argsort(prediction[0])[::-1]
    scores = prediction[0][indices]
    labels = [CLASS_LABELS[i] for i in indices]
    ...
    ax.invert_yaxis()
    ax.set_title("Confidence Scores")
    img_path = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4().hex}_confidence.png")
...
def save_bulk_pdf(reports):
    ...
    for report in reports:
        ...
        c.drawString(50, 780, "Leukemia Diagnostic Report")
        c.setFont("Helvetica-Bold", 12)
        c.drawString(50, 760, "Date:")
        c.setFont("Helvetica", 12)
        c.drawString(110, 760, f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")

        c.setFont("Helvetica-Bold", 12)
        c.drawString(50, 740, "Prediction:")
        c.setFont("Helvetica", 12)
        c.drawString(130, 740, report['prediction'])

        c.setFont("Helvetica-Bold", 12)
        c.drawString(50, 720, "Confidence:")
        c.setFont("Helvetica", 12)
        c.drawString(130, 720, f"{report['confidence']:.2f}%")

        # Convert images to PIL and save
        temp_img_path = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4().hex}_original.jpg")
        temp_img_path1 = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4().hex}_heatmap.jpg")
        temp_img_path2 = os.path.join(tempfile.gettempdir(), f"{uuid.uuid4().hex}_overlay.jpg")
        temp_plot_path = save_confidence_plot(report['plotGraph'])

        report['image'].save(temp_img_path)
        Image.fromarray(np.uint8(report['heatmap'] * 255)).convert("RGB").save(temp_img_path1)
        Image.fromarray(report['overlayed']).convert("RGB").save(temp_img_path2)

        # Draw images in the PDF in a row
        c.drawImage(temp_img_path, 50, 500, width=160, height=160)
        c.drawImage(temp_img_path1, 220, 500, width=160, height=160)
        c.drawImage(temp_img_path2, 390, 500, width=160, height=160)
        c.drawImage(temp_plot_path, 150, 300, width=300, height=120)

        c.setFont("Helvetica-Bold", 10)
        c.drawString(80, 470, "Left: Original Image ")
        c.drawString(230, 470, "Center: Grad-CAM Heatmap")
        c.drawString(420, 470, " Right: Overlay")

        # Draw interpretation text below the image area
        c.setFont("Helvetica", 10)
        text_object = c.beginText(50, 270)
        for line in report["text"].split("\n"):
            stripped = line.strip()
            if stripped.startswith("**") and stripped.endswith("**"):
                c.setFont("Helvetica-Bold", 10)
                text_object.textLine(stripped.strip("*"))
                c.setFont("Helvetica", 10)
            else:
...
def main():
    ...
    with st.sidebar:
        st.image(os.path.join(os.path.dirname(os.path.abspath(__file__)), "Assests", "red-blood-cells.png"), width=150)
        st.header("About")
        st.markdown("""
    ...
    if uploaded_images:
        ...
        for filename, img in uploaded_images:
            ...
            # Interpretation markdown stored in a variable
            INTERPRETATION_TEXT = lambda pred_class: f"""
            ...
            **Clinical correlation:**
            ...
            st.markdown(INTERPRETATION_TEXT(pred_class))

            reports.append({"image": enhanced_img, "prediction": pred_class, "confidence": confidence,"heatmap":heatmap,"overlayed":overlayed,"text":INTERPRETATION_TEXT(pred_class),"plotGraph":prediction})
        ...
        if st.button("📄 Download Combined PDF Report"):
            ...
            with open(pdf_path, "rb") as f:
                st.download_button("Download All Reports as PDF", f, file_name=f"combined_leukemia_report_{uuid.uuid4().hex}.pdf", mime="application/pdf")
